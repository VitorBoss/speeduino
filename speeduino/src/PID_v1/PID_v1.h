...
public:
  ...
  #define DIRECT 0
  #define REVERSE 1
  #define uPID_SHIFTS 15

  //commonly used functions **************************************************************************
  uPID(long *, long *, long *,  // * constructor.  links the PID to the Input, Output, and
       long, long, long, byte); //   Setpoint.  Initial tuning parameters are also set here
  ...
  bool ComputeNow(bool, long FeedForwardTerm = 0); // * performs the PID calculation intantly
...
private:
  ...
  long *mySetpoint;
  unsigned long lastTime;
  long outMin, outMax, lastOut;

  uint16_t SampleTime;
  byte controllerDirection, shifts;
  bool inAuto;
};
